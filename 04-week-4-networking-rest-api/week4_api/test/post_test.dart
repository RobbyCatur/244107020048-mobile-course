...
/// Repository palsu: mengganti PostRepository di provider sehingga test
/// berjalan tanpa internet sama sekali (Dio tidak pernah dipakai).
/// Pola override ini jadi fondasi mock API di Minggu 12 (Testing & QA).
...
void main() {
    ...
    final container = ProviderContainer(
      overrides: [
        postRepositoryProvider.overrideWith(
          (ref) => FakePostRepository(items: const [
            Post(
  ...
  test('provider error dengan repository palsu', () async {
    final container = ProviderContainer(
      overrides: [
        postRepositoryProvider.overrideWith(
          (ref) => FakePostRepository(throwError: true),
        ),
      ],
    );
    addTearDown(container.dispose);
